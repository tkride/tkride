    ...
    filter_tickers(filter) {
        ...
        if(this.brokers) {
            $.each(this.brokers, function(broker) {
                ...
                if((that.brokers[broker] != undefined) && (that.brokers[broker].tickers.length > 0)) {
                    tickers[broker] = that.brokers[broker].tickers.filter(ticker => ticker.startsWith(filter)).sort();
